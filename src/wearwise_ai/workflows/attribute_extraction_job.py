...
from wearwise_ai.application.attribute_extraction_service import (
    ...
    ATTRIBUTE_EXTRACTION_MODEL_NAME,
    ATTRIBUTE_EXTRACTION_MODEL_VERSION,
    ATTRIBUTE_EXTRACTION_PROCESSING_VERSION,
...
class AttributeExtractionJobOutcome:
    request_id: str
    job_id: str
    asset_id: str
    status: str
    result: AttributeExtractionResult | None


class AttributeExtractionJobWorkflow:
    def __init__(
        self,
        *,
        ...
        self._attribute_extraction_service = attribute_extraction_service
    ...
    def run(self, *, job_id: str, worker_id: str, request_id: str) -> AttributeExtractionJobOutcome:
        ...
        if background_removed.get("review_required") is True:
            ...
            return AttributeExtractionJobOutcome(
                request_id=request_id,
                job_id=job_id,
                asset_id=asset_id,
                status="needs_review",
        ...
        if artifact_location is None:
            self._internal_api.fail_job(
                ...
                payload={
                    "worker_id": worker_id,
                    "error_code": "ATTRIBUTE_EXTRACTION_INPUT_MISSING",
                    "error_message": "Cleaned garment image access URL is missing.",
                    "status_message": "attribute extraction input missing",
                    "retryable": False,
                },
            )
            return AttributeExtractionJobOutcome(
        ...
        if not validate_cleaned_artifact_or_fail(
            ...
            error_code="ATTRIBUTE_EXTRACTION_INPUT_INVALID",
            status_message="attribute extraction input invalid",
        ):
            return AttributeExtractionJobOutcome(
                request_id=request_id,
                job_id=job_id,
        ...
        self._internal_api.report_progress(
            ...
            worker_id=worker_id,
            progress_percent=75,
            status_message="extracting garment attributes",
        )
        result = self._attribute_extraction_service.extract(
            AttributeExtractionRequest(
                asset_id=asset_id,
                image_bytes=image_bytes,
                content_type=content_type,
                category_code=_category_code(options),
            )
        )
...
def _category_code(options: object) -> str | None:
    if not isinstance(options, dict):
        return None

    value = options.get("category_code")
...
def _review_required_payload(
    *,
    ...
    return {
        ...
        "model_name": ATTRIBUTE_EXTRACTION_MODEL_NAME,
        "model_version": ATTRIBUTE_EXTRACTION_MODEL_VERSION,
        "processing_version": ATTRIBUTE_EXTRACTION_PROCESSING_VERSION,
        "status_message": "attribute extraction skipped; background removal needs review",
        "result": {
            "brand": None,
            "subcategory": None,
        ...
        "confidence": None,
        "resource_metrics": {
            "attribute_extraction_status_reason": "background_removal_needs_review",
